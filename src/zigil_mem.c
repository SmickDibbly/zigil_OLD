...
#ifdef ZGL_LINUX
    alloc_bytes += malloc_usable_size(new_ptr);
#endif
    return new_ptr;
}
...
static void dump_records(void) {
    ...
    while (entry != &record_list) {
        ...
        leaked_byte_count += rec->req_size;
        entry = entry->next;
    }
...
static void *create_MemRecord(const char* filestr, int line, size_t req_size) {
    ...
    num_allocs++;
#ifdef ZGL_LINUX
    alloc_bytes += malloc_usable_size(rec);
    user_alloc_bytes += req_size;
