 ...
 int n) {
    ...
    for (int i = 0; i < n; i++) {
        if (((poly.pt[i].y <= P.y) && (poly.pt[i+1].y > P.y))
            ...
            fix32_t vt = FIX_DIV(P.y - poly.pt[i].y, poly.pt[i+1].y - poly.pt[i].y);
...
(zgl_mPixel p0,
 ...
 zgl_mPixel *ip1) {
    ...
    fix32_t t_in = fixify(0);
    fix32_t t_out = fixify(1);
    fix32_t t;
    zgl_mPixel SV;
    zgl_mPixel LV;
    zgl_mPixel EV;
    ...
    ip0->x = (p0.x) + FIX_MUL(t_in, SV.x);
    ip0->y = (p0.y) + FIX_MUL(t_in, SV.y);
    ip1->x = (p0.x) + FIX_MUL(t_out, SV.x);
    ip1->y = (p0.y) + FIX_MUL(t_out, SV.y);
...
(Fix32Point_2D pt0, Fix32Point_2D pt1,
 ...
 fix32_t ymin, fix32_t ymax) {
    ...
    *c1 = pt1;
    ...
    // defining variables
    fix32_t neg_dx = -(x1 - x0);
    fix32_t pos_dx = -neg_dx;
    fix32_t neg_dy = -(y1 - y0);
    ...
    if (neg_dx != 0) {
        fix32_t r0 = FIX_DIV(q0, neg_dx);
        fix32_t r1 = FIX_DIV(q1, pos_dx);
        if (neg_dx < 0) {
    ...
    if (neg_dy != 0) {
        fix32_t r2 = FIX_DIV(q2, neg_dy);
        fix32_t r3 = FIX_DIV(q3, pos_dy);
        if (neg_dy < 0) {
            negarr[negind++] = r2;
            posarr[posind++] = r3;
        } else {
